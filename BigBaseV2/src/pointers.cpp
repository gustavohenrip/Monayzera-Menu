...
namespace big
{
	pointers::pointers()
	{
		...
		main_batch.add("NSM", "48 8B 0D ? ? ? ? 41 8A D9", [this](memory::handle ptr) {
			m_network_shopping_mgr = ptr.add(3).rip().as<decltype(m_network_shopping_mgr)>();
			});

		main_batch.add("CB", "E9 7E E9 BF 02 48 8B 41 10 8B 08 F3 0F 10 48 08", [this](memory::handle ptr) {
			m_construct_basket = ptr.as<decltype(m_construct_basket)>();
			});

		main_batch.add("AITB", "E9 AC A0 6E 02 48 8D 64 24 F8 48 89 04 24 E9 E0", [this](memory::handle ptr) {
			m_add_item_to_basket = ptr.as<decltype(m_add_item_to_basket)>();
			});

		main_batch.add("BS", "E9 F7 0C 63 03 48 8D 15 A8 A2 D9 FF 48 B9 22 A2", [this](memory::handle ptr) {
			m_begin_service = ptr.as<decltype(m_begin_service)>();
			});
